...
\begin{remark} \label{altdim}
	Another way of tackling the intuition for a fractal dimension for discrete locally finite sets is the following which is motivated by \cite[Part II Page 98]{fractalwinter} and \cite[Page 83]{lawler}. Let $M\subset \R^d$ be a locally finite set. Let $n\in\N$ and $B_n$ be the ball or radius $n$ and center $0$. We could assign $M$ the dimension $d_M$ if its cardinality $|M\cap B_n|$ inside the ball $B_n$ is of order $n^{d_M}$ for large $n$, so we could define
	\begin{flalign*}
		d_M := \liminf_{n\to\infty} \frac{\ln(|M \cap B_n|)}{\ln(n)}. 
	\end{flalign*}
	If we apply that definition to our final cluster $\E_\infty$ it is not clear whether this definition is equivalent to (\ref{fractaldimension}). If we define the fractal dimension as in (\ref{fractaldimension}) we encounter the problem that when the cluster reaches some radius for the first time, then in many instances in the subsequent steps there will particles be added to the cluster without increasing its radius, so to say \glqq $\text{filling}$\grqq\ the cluster up to that radius (see \autoref{notfilled}). If the fractal dimension shall indicate an asymptotic relation between how many particles of the final cluster $\E_\infty$ lie in a ball of a radius letting the radius tend to infinity, the definition in (\ref{fractaldimension}) does not reflect that directly. Nevertheless it seems to make sense that these two situations are equivalent which we state more clearly in the following conjecture. \\
...
\begin{proof}
	...
	\begin{flalign*}
		d_f = \frac{1}{\alpha_f}\quad \text{a.s.}
	\end{flalign*}
	where $\alpha_f$ is the growth rate as defined in section (\ref{growthrate}), and we will therefore show that
	\begin{flalign*}
...
\begin{flalign*}
	d_f = \frac{d^2 + 1}{d+1}
\end{flalign*}
in $\Z^d$ which seems to agree fairly well with simulations which you can find in \cite{vicsek}. We will proof in the following an almost sure lower bound for the fractal dimension of external DLA for which we will use theorem \ref{iatheorem}. Recall that $\mP_f := \mP_f^2$. 
...
\begin{definition} \label{ghitA}
	Let $g=g_{\alpha,p}\in \G$ and $A\in \mP_f$. For the context in this chapter we define 
	
	\begin{align*}
		g\cap A := \{ p\in A\ |\ g \cap sq(p)\neq \emptyset\}
	\end{align*}
	...
	\end{cases}
	\end{align*}\\
	It is easy to calculate that this defines a well-defined relation on $g\cap A$. In the following we will prove that this relation is totally ordered. 
\end{definition}
...
\begin{theorem} \label{uniqmeas}
	On $A(d,q)$ there exists an unique $G_d$-invariant Radon measure $\mu_q$ such that
	\begin{flalign}
...
By theorem \ref{uniqmeas} we know that $\mu$ is, up to a factor, the only Euclidean motion invariant measure on $\G$. Since it is locally finite, we can define for any $K\in \K^2$ a probability measure on $\G$ by
\begin{flalign*}
...
\end{flalign*}
and since $\bigcup_{p\in A} sq(p)$ is a bounded set we have $conv(A)\in \K^2$. Recall the definition 
\begin{flalign*}
	[B] := \{g\in\G\ |\ g\cap B \neq \emptyset\}
\end{flalign*}
