...
\subsubsection{Graphs} \label{zgraph}
...
\begin{flalign*}
	\mP^d_f := \{A\subset \mathbb{Z}^d\ |\ \text{A is finite}\}, 
\end{flalign*}
...
\begin{definition} $\mathit{(external\ diffusion\ limited\ aggregate)}$ $\mathit{External\ diffusion\ limited}$ $\mathit{aggregate}$ (on $\Z^d$), short $\mathit{external\ DLA}$ or just $\mathit{DLA}$, is an incremental aggregate with the family of harmonic measures $(h_A)_{A\in\mP^d_f}$ as distribution. 
\end{definition}
...
\begin{proposition} \label{general}
	Theorem \ref{keytheorem} can be generalized to the following. There exists a $c>0$ such that for all $y\in\Z^2$ and $s,r\in[1,\infty)$ with $s\geq r$ we have 
	\begin{flalign*}
...
\begin{proof}
	...
	\begin{flalign*}
		...
		&= h_{\Phi_y(A)}(\Phi_y(z)) 
	\end{flalign*}
	for all $z\in A$. By Theorem \ref{keytheorem} we know that there exists a $c>0$ such that for all $s\in[1,\infty)$ 
	\begin{flalign*}
...
\begin{proof}
	...
	\begin{flalign} \label{lambda}
		\PP(D_c) = 1. 
	\end{flalign}
	Note that $h$ is bijective, multiplicative and monotonously growing. Lets first argument why the inequality at $(+)$ indeed holds. We define CONTINUE
	...
	\begin{flalign*}
		...
		\end{cases}
	\end{flalign*}
	so $\sigma_i$ is the waiting time between adding $z_i$ and $z_{i+1}$ to the cluster, if both are added. We quickly argument that $\sigma_i$ is measurable aswell. For $j\in\N$ we have
	\begin{flalign*}
	...
	\\Let $i\in \{1,\dots,n-1\}$ and define
	\begin{flalign*}
	...
	\begin{flalign} \label{geom}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign}
	for some constant $c_1>0$, which is the same constant for all $i\in \{1,\dots,n-1\}$ and for all $n\in\N$. We will want to use Proposition \ref{general} here, for which we need to create terms where the harmonic measure appears. For that we need probabilities which condition on a given cluster which we will develop now. We first define some helpful sets. For $m\in\N$ define 
	\begin{flalign*}
	...
	\begin{flalign*}
		E_m := \{\E\in C\ |\ m-1\leq \rad(\E)\leq m\} 
	\end{flalign*}
	such that $C=\bigcup_{m\in\N} E_m$ aswell. Since $|z_1| \geq n$ and considering $U_{[z]}^i$, we need a minimal amount of time steps such that it is possible for the cluster to contain $z_i$. So there exists a $n_0\in\N$ such that 
	\begin{flalign*}
	...
	\begin{flalign*}
		\PP(\E_{j-1}=\E,\tau_i=j,U_{[z]}^i) \begin{cases}
			>0, \quad \text{ for }\E\in E_m \text{ and }m\geq m_0,\\
			=0, \quad\text{ for }\E\in E_m \text{ and }m< m_0. 
		\end{cases}
	\end{flalign*}
	Having made that clear we can use the law of total probability twice in the following. For $k\in\N_0$ we get 
	\begin{flalign*}
		\PP(\sigma_i > k\ |\ U_{[z]}^i) &= \sum_{n_0\leq j \leq\infty}\PP(\tau_i=j\ |\ U_{[z]}^i) \PP(\sigma_i > k\ |\ \tau_i=j,U_{[z]}^i) \\
		&\geq \sum_{n_0\leq j <\infty}\PP(\tau_i=j\ |\ U_{[z]}^i) \PP(\sigma_i > k\ |\ \tau_i=j,U_{[z]}^i).
	\end{flalign*}
	For shorter expressions write $\Gamma_{ij}:=\{\E_{j-1} = \E\} \cap \{\tau_i=j\} \cap U_{[z]}^i$. Further for $n_0\leq j <\infty$ we get 
	\begin{flalign*}
		\PP(\sigma_i > k\ |\ \tau_i=j,U_{[z]}^i) = \sum_{m_0\leq m} \sum_{\E\in E_m}  \PP(\E_{j-1}=\E\ |\ \tau_i=j,U_{[z]}^i)\PP(\sigma_i > k\ |\ \Gamma_{ij}). 
	\end{flalign*}
	We now state that there exists a constant $c_1>0$ such that for all $k\in\N$, $n_0\leq j <\infty$, $m_0\leq m$ and $\E\in E_m$ we have
	\begin{flalign} \label{induc}
		\PP(\sigma_i > k\ |\ \Gamma_{ij}) \geq (1-c_1n^{-\frac{1}{2}})^k
	\end{flalign}
	which we prove by induction. For $k=1$ by Proposition \ref{general} there exists a $c>0$ such that 
	\begin{flalign*}
		\PP(\sigma_i > 1\ |\ \Gamma_{ij}) &\geq \PP(\tau_{j+1} \neq z_{i+1}\ |\ \Gamma_{ij}) \\
		&= 1 - h_{\E\cup \{z_i\}}(z_{i+1}) \\
		&\geq 1 - cn^{-\frac{1}{2}} \\
		&=(1 - cn^{-\frac{1}{2}})^1. 
	\end{flalign*} 
	...
	\begin{flalign*} 
		...
		&= (1-cn^{-\frac{1}{2}})^{l+1},
	\end{flalign*}
	where in $(+)$ we used the induction assumption. In order to show $(++)$ we would need to split the probability with the law of total probability by conditioning on all clusters such that $\Gamma_{ij}$ is fulfilled and the next $l$ added points are not equal to $z_{i+1}$. We would then have the same inequality as in the induction beginning using Proposition \ref{general} and reput together the contioned probabilites to get this inequality here. Same as in the induction beginning we have now found a constant $c_1>0$ such that (\ref{induc}) holds for all $k\in\N$ and this $c_1$ is independent of $n_0\leq j <\infty$, $m_0\leq m$ or $\E\in E_m$. Therefore we can go backwards the two splittings where we used the law of total probability and finally get 
	\begin{flalign*}
		\PP(\sigma_i > k\ |\ U_{[z]}^i) \geq (1-c_1n^{-\frac{1}{2}})^k
	\end{flalign*}
	with a constant $c_1>0$ which doesn't depend neither on $i\in \{1,\dots,n-1\}$ or $n\in\N$ by the strong universal property given by Proposition \ref{general}. We therefore have shown what we stated at (\ref{geom}). \\
	...
	PROBLEM
	\begin{flalign*}
		\PP(\tau_{n} \leq \tilde m_n\ |\ U_{[z]}^n) \leq \PP(\tau_{n} - \tau_1 \leq \tilde m_n\ |\ U_{[z]}^n) = \PP(Y \leq \frac{an}{p_n}\ |\ U_{[z]}^n) \leq (e^2a)^{n-1}
	\end{flalign*}
...
\begin{remark}
	With this theorem and \ref{fractaldim} we conclude that the fractal dimension of external DLA is at least $\frac{3}{2}$. In the next chapter we will look at another incremental aggregate which tries to approximate DLA and compare simulations of both to get an empirical comparision of their growth rates. 
\end{remark}
...
\begin{definition} \label{linehittingdistribution}
	...
	\end{flalign*}
	Hence, the family of distributions $(\mu_A)_{A\in \mP_f}$ defines an incremental aggregate. 
\end{definition}
...
\\
Note again that you can also run an external DLA simulation but we do not claim that it is mathematically precise and therefore won't argument that in this paper. The parameters in $\mathit{parameters.json}$ are read and depending on which process (LHA or DLA) shall be simulated, the according object gets initiated and its $\mathit{run\_process}$ method gets executed. The init method of $\mathit{Line\_Hitting\_Aggregate}$ looks like the following:\\
\\
...
CONTINUE\\
\\I hope that I have raised interest in the topic of incremental aggregates with this paper. I feel delighted to watch at the beautiful realizations of the aggregates and I hope the reader can feel similar looking at them and realizing images him- and herself. 
