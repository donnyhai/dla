...
\begin{proof}
	...
	We also define
	\begin{flalign*}
		U_{[z]}^n = \{\tau_1<\tau_2<\dots<\tau_n<\infty\}
	\end{flalign*}
	and
	\begin{flalign*}
		u_{[z]}^n := \PP(U_{[z]}^n)
	\end{flalign*}
	which play an important role in proof later. 
	EVTL UNNÖTIG
	...
	\\Let $i\in \{1,\dots,n-1\}$. We will now prove that the distribution of $\sigma_i$ is bounded by that of a geometrically distributed random variable with parameter
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
	for some constant $c_1>0$, which is the same constant for all $n\in\N$. 
	We will want to use Proposition \ref{general} here, for which we need to create terms where the harmonic measure appears. For that we need probabilities which condition on a given cluster which we will develop now. We first define some helpful sets. For $m\in\N$ define 
	\begin{flalign*}
	...
	\end{flalign*}
	such that $C=\bigcup_{m\in\N} E_m$ aswell.
	Since $|z_1| \geq n$ and $z_i$ is reachable by $z_1$ in $i-1$ steps we need some minmal amount of time steps such that it is possible for the cluster to contain $z_i$. So there exists a $n_0\in\N$ such that 
	\begin{flalign*}
		\PP(\tau_i=j) \begin{cases}
			>0, \quad \text{ for }j\geq n_0,\\
			=0, \quad\text{ for }j< n_0. 
		\end{cases}
	\end{flalign*}
	Further if we choose $j\geq n_0$, then $\E_{j-1}$ must have some minimal radius if we want it to be possible that the next added point is $z_i$, similarly argumented as above. So there exists a $m_0\in\N_0$ such that
	\begin{flalign*}
		\PP(\E_{j-1}=\E,\tau_i=j) \begin{cases}
			>0, \quad \text{ for }\E\in E_m \text{ and }m\geq m_0,\\
			=0, \quad\text{ for }\E\in E_m \text{ and }m< m_0. 
		\end{cases}
	\end{flalign*}
	Having made that clear we can use the law of total probability twice in the following. For $k\in\N_0$ get 
	\begin{flalign*}
		\PP(\sigma_i > k) &= \sum_{n_0\leq j \leq\infty}\PP(\tau_i=j) \PP(\sigma_i > k\ |\ \tau_i=j) \\
		&=\PP(\tau_i=\infty) + \sum_{n_0\leq j <\infty}\PP(\tau_i=j) \PP(\sigma_i > k\ |\ \tau_i=j), 
	\end{flalign*}
	note that $\PP(\sigma_i > k\ |\ \tau_i=\infty)=1$ since $\{\tau_i=\infty\}\subset\{\sigma_i=\infty\}\subset\{\sigma_i>k\}$. Further for $n_0\leq j <\infty$ we get 
	\begin{flalign*}
		\PP(\sigma_i > k\ |\ \tau_i=j) = \sum_{m_0\leq m} \sum_{\E\in E_m}  \PP(\tau_i=j\ |\ \E_{j-1}=\E)\PP(\sigma_i > k\ |\ \E_{j-1} = \E, \tau_i=j). 
	\end{flalign*}
	We now state that there exists a constant $c_1>0$ such that for all $k\in\N$, $n_0\leq j <\infty$, $m_0\leq m$ and $\E\in E_m$ we have
	\begin{flalign*}
		\PP(\sigma_i > k\ |\ \E_{j-1} = \E, \tau_i=j) \geq (1-c_1n^{-\frac{1}{2}})^k
	\end{flalign*}
	which we prove by induction. For shorter expressions write $\gamma_{ij}:=\{\E_{j-1} = \E\} \cap \{\tau_i=j\}$. For $k=1$ by Proposition \ref{general} there exists a $c>0$ such that 
	\begin{flalign*}
		...
		&=( 1 - cn^{-\frac{1}{2}})^1. 
	\end{flalign*} 
	Note that we can use Proposition \ref{general} since ... Note that the harmonic measure doesn't care about the order of how points where added to form the current cluster, as mentioned in Remark \ref{orderindependence}. Now let the statement be true for some $k=l$. Then again by Proposition \ref{general} there exists a $c>0$ such that 
	\begin{flalign*}
		\PP(\sigma_i > l+1\ |\ \gamma_{ij}) &= \PP(\sigma_i > l, y_{l+1} \neq z_{i+1}\ |\ \gamma_{ij}) \\
		&= \PP(\sigma_i>l\ |\ \gamma_{ij}) \PP(y_{l+1} \neq z_{i+1}\ |\ \gamma_{ij}, \sigma_i>l) \\
		&\overset{(+)}{\geq} (1-c_1n^{-\frac{1}{2}})^l \PP(y_{l+1} \neq z_{i+1}\ |\ \gamma_{ij}, \sigma_i>l) \\
		&\overset{(++)}{\geq} (1-c_1n^{-\frac{1}{2}})^l(1-c_1n^{-\frac{1}{2}}) \\
		&= (1-c_1n^{-\frac{1}{2}})^{l+1},
	\end{flalign*}
	where in $(+)$ we used the induction assumption. In order to show $(++)$ we would need to split the probability by conditioning on all clusters such that 
	...\\
	INDUCTION END\\
