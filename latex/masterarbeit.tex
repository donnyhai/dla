...
\begin{lemma} \label{randt}
	Let $a>0$ and $h:[0,\infty) \to [0,\infty)$ a bijective function. Then the following are equivalent:
	\begin{enumerate}
		\item $\{\exists c>0: r(n) \leq ch(n) \}\text{ a.s. for large } n$
		\item $\{\exists c>0: T(ar)\geq ch^{-1}(r)\} \text{ a.s. for large } r$
	\end{enumerate}
\end{lemma}

\begin{proof}
	$\Rightarrow$: \\
	Take $N\in\N$ such that $\exists c>0$ with $r(n)\leq ch(n)$ for all $n>N$. 
\end{proof}
...
\begin{proof}
	...
	\end{flalign}
	for large $n$ a.s., where $\lambda_n := \min\{j\in\N\ |\ rad(E_j)\geq n\}$ for $n\in \N$. ($\ref{rad}$) then follows easily from ($\ref{lambda}$). For $n\in\N$ write $\E_n = \{y_1,\dots,y_n\}$ according to the definition in $\ref{incrementalaggregate}$, where $y_j$ is the $j$-th point added to the cluster. Let $\beta > 0$. For $n\in\N$ let $\tilde m_n := \beta n^{\frac{3}{2}}$ and define 
	\begin{flalign*}
		V_n := \{\lambda_{2n} \leq \tilde m_n\} \subset \Omega. 
	\end{flalign*}
	...
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
	for some constant $c_1>0$. With Lemma \ref{geometric} CONTINUE
