...
\begin{definition} $\mathit{Random\ Line\ Hitting\ Distribution}$\\
	Choose $A\in \mathcal{P}_f$. We define a distribution $\mu_A$ on $\mathbb{Z}^2$ as in the following. 
	Let $L=L_{a,b}$ be a random line according to the line measure $\mu_0$ with the condition that $L$ hits $A$. Then define a variable , which chooses uniformly an element out of $\{min_{<_{line}}L_A, max_{<_{line}}L_A\}$. Then $\mu_A := U_{L_A}$ we will call the $\mathit{Random\ Line\ Hitting\ Distribution\ (of\ A)}$.
\end{definition}
