...
\begin{proof}
	...
	\end{flalign}
	for large $n$ a.s., where $\lambda_n := \min\{j\in\N\ |\ rad(E_j)\geq n\}$ for $n\in \N$. ($\ref{rad}$) then follows easily from ($\ref{lambda}$). For $n\in\N$ write $\E_n = \{y_1,\dots,y_n\}$ according to the definition in $\ref{incrementalaggregate}$ with $y_0 := 0$, where $y_j$ is the $j$-th point added to the cluster. Let $\beta > 0$. For $n\in\N$ let $\tilde m_n := \beta n^{\frac{3}{2}}$ and define 
	\begin{flalign*}
	...
	define events 
	\begin{flalign*}
		W_n([z]) := \{\exists j_1< \dots < j_n \leq \tilde m_n \ \text{ s.t. } z_i = y_{j_i} \text{ for all } i\in\{1,\dots,n\} \} \subset \Omega
	\end{flalign*}
	...
	\end{flalign}
	so we can follow that $\PP(V_n^C)=1$ for large $n$ and nearly have what we want to proof. For that take $n\in \N$ and $[z]\in Z_n$ and define
	\begin{flalign*}
