...
\begin{definition} \label{incrementalaggregate}
	Let $\mu=(\mu_A)_{A\in \mathcal{P}_f}$ be a family of distributions with $\mu_A\in \mathcal{D}_A$ for all $A\in \mathcal{P}_f$. An $\mathit{Incremental\ Aggregate\ (with\ distribution\ \mu)}$ is a stochastic process $(\mathcal{E}_n)_{n\in{\mathbb{N}}}$ which evolves as follows. The process starts with one point $\mathcal{E}_1 = \{0\}$ at the origin of $\mathbb{Z}^d$. Knowing the process $\mathcal{E}_n$ after having added $n$ points, let $y_{n+1}$ be a random point in $\partial \mathcal{E}_n\in \mathcal{P}_f$ with distribution
	\begin{align}
...
The above defines a Markov chain whose state space is the set of finite and connected subets of $\Z^d$. For all incremental aggregates in this paper we will focus on the $2$-dimensional case $\Z^2$ and will always identify $\R^2$ with $\C$ for a more convenient notation. 
...
\begin{proof}
	...
	\end{flalign}
	for large $n$ a.s., where $\lambda_n := \min\{j\in\N\ |\ rad(E_j)\geq n\}$ for $n\in \N$. ($\ref{rad}$) then follows easily from ($\ref{lambda}$). For $n\in\N$ write $\E_n = \{y_0,\dots,y_{n-1}\}$ according to the definition in $\ref{incrementalaggregate}$ with $y_0 := 0$, where $y_j$ is the $j$-th point added to the cluster. Let $\beta > 0$. For $n\in\N$ let $\tilde m := \beta n^{\frac{3}{2}}$ and define 
	\begin{flalign*}
		V_n := \{\lambda_{2n} \leq \tilde m\} \subset \Omega. 
	\end{flalign*}
	...
	define events 
	\begin{flalign*}
		W_n([z]) := \{\exists j_1< \dots < j_n \leq \tilde m \ \text{ s.t. } z_i = y_{j_i} \text{ for all } i\in\{1,\dots,n\} \} \subset \Omega
	\end{flalign*}
	and the union of these events 
	\begin{flalign*}
		W_n := \bigcup_{[z] \in Z_n} W_n([z]). 
	\end{flalign*}
	We will quickly proof that $V_n \subset W_n$ for all $n\in\N$. Let $\omega \in V_n$, then $\lambda_{2n}(\omega) \leq \tilde m$. For $m:=\max\{j\in\N\ |\ j\leq \tilde m\}$ we therefore have $rad(\E_m(\omega)) \geq 2n$ (Note that $\lambda_{2n}(\omega)\in\N$). Therefore since $\E_m(\omega)$ is connected there must exist indices $j_1 <\dots < j_n$ s.t. $y_{j_1}(\omega)\in B_n$ and $[z_0] := [y_{j_1}(\omega),\dots, y_{j_n}(\omega)]\subset \E_m(\omega)$, since $\max_{x\in [z_0]} |x| \leq 2n$. Therefore $j_n \leq m\leq\tilde m$ and therefore $\omega \in W_n([z_0]) \subset W_n$ which proofs the inclusion. For a sequence of events $(A_n)_{n\in\N}$ define
	\begin{flalign*}
		\limsup_{n\to\infty} A_n := \bigcap_{n\in\N} \bigcup_{i\geq n} A_i = {\omega\in\Omega\ |\ A_n \text{ holds for infinetly many } n\in\N}.
	\end{flalign*}
	We will want to use the Lemma of Borel-Cantelli on the sequence of events $(W_n)_{n\in\N}$ to get
	\begin{flalign}
		\PP(\limsup_{n\to\infty} V_n) \leq \PP(\limsup_{n\to\infty} W_n) = 0, 
	\end{flalign}
	so we can follow that $\PP(V_n^C)=1$ for large $n$ and nearly have what we want to proof. For that take a $n\in \N$ and a $[z]\in Z_n$ and define
	\begin{flalign*}
		\tau_i \text{ the index s.t. } y_{\tau_i} = z_i
	\end{flalign*}
	and
	\begin{flalign*}
