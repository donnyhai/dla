...
Let $(\Omega,\mathcal{F}, \mathbb{P})$ be a probability space. For our space of interest $\mathbb{Z}^d$ we will always use the discrete $\sigma$-algebra which is the power set of $\mathbb{Z}^d$. If for $A\in \mathcal{F}$ we have $\mathbb{P}(A)=1$ we will say that "$A$ holds $\mathbb{P}$-$a.s.$", or short "$A$ holds $a.s.$" ($A$ holds almost sure).
...
\noindent Note that $|N(z)| = 2d$ for all $z\in \mathbb{Z}^d$ since every point has two neighbours in every dimension. We can therefore follow easily that $\mathbb{P}(S_n = y\ |\ S_{n-1} = z) = 0$ for all $y\notin N(z)$ and $z\in \Z^d$. So a Random Walk can be understood as a particle starting from some point $x$ and moving randomly on the grid choosing its next step uniformly from its neighbours. For the following let $(S_n)_{n\in \mathbb{N}}$ be a random walk on $\Z^d$ starting at $x\in \Z^d$. 
...
Looking at computer simulations of External DLA it seems that the cluster are formed relatively sparse and they appear to have anoninteger fractal dimension. The notion of fractal dimension is not very clear but the following bulids on a intuitive feel of what the "dimension" of a subset of $\Z^d$ should be. First we define the radius of a finite set $A\subset \Z^d$ as 

\begin{flalign*}
	rad(A) := \max_{x\in A} |x|. 
\end{flalign*}

For an Incremental Aggregate $(\E_n)_{n\in{\mathbb{N}_0}}$ we have that $\E_n$ contains $n$ points and for the notion of dimension we are interested in how the cardinality of $\E_n$ and its radius relate to each other. We understand the fractal dimension as a value $\bar d$ such that

\begin{flalign*}
	n\approx rad(\E_n)^{\bar d}. 
\end{flalign*}
...
\subsection{Fractal Dimension and Growth rate}
...
\begin{proof}
	...
	\end{flalign*}
	and therefore $R\in \sigma(\mathcal{R}_+)\subset\sigma(\mathcal{R})$. Similairly it works if $a,b<0$. If $0\in[a,b]$ then we can write $R = [\alpha,\beta]\times [a,b]$ with three components $R = [\alpha,\beta] \times [a,0) \cup [\alpha,\beta] \times \{0\} \cup [\alpha,\beta] \times (0,b]$ which lie in $\mathcal{R}_-, \mathcal{R}_0$ and $\mathcal{R}_+$ respectively. Therefore $R\in \sigma(\mathcal{R})$ aswell. By measure theory the above described rectangles form a generator set of $\mathcal{B}_\Phi$, which completes the proof. 
\end{proof}
