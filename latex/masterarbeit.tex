...
\begin{proof}
	...
	\begin{flalign*}
		\tau_i:\Omega \to \N^\infty, \tau(\omega) = j :\Leftrightarrow \begin{cases}
	...
	\\Choose $\omega\in W_n([z])$ and take $i\in\{1,\dots,n-1\}$. Note that in this case $\tau_j(\omega)$ is finite for all $j\in\{1,\dots,n\}$. 
	
	
	
	
	Since $\E_{j_i}(\omega)$ is connected and contains $0$, by Theorem $\ref{keytheorem}$ we get that the distribution of $\sigma_i$ is bounded by that of a geometrically distributed random variable with parameter
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
