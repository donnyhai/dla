...
\end{lemma}
\begin{proof}
	To proof this it is helpful to consider generators of these $\sigma$-algebras. Define the set of closed rectangles in $\Phi$ as $R := \{[\alpha,\beta] \times [c,d]\subset \C\ |\ 0\leq \alpha<\beta<\pi,c<d\}$. Then by measure theory we know that $\mathcal{B}_\Phi = \sigma (R)$ and since $\chi$ is a bijection, we have $\chi(\sigma (R)) = \sigma (\chi(R))$ and finally $\GG = \sigma(\chi(R))$. For $\tilde A := \{\bar K\ |\ K\in \K^2\}$ we have by definition $\mathcal{A}(2,1) = \sigma(\tilde A)$. \\
	\\ \indent $\subset$: Let $K\in\K^2$. We will show that $\chi^{-1}(\bar K)$ is a closed set in $\Phi$. If that is the case we can write $\chi^{-1}(\bar K) = \cup_{i=1}^\infty R_i$ for some rectangles $R_i\in R$, hence $\bar K =  \cup_{i=1}^\infty \chi(R_i)$ and therefore $\bar K\in\sigma(\chi(R)) = \GG$. Therefore we have $\tilde A\subset \GG$ and finally $\sigma(\tilde A) = \mathcal{A}(2,1) \subset \GG$. To show that $\chi^{-1}(\bar K)$ is a closed let $(\alpha_0,p_0)\in \Phi\setminus \chi^{-1}(\bar K)$. Then $\chi(\alpha_0,p_0) \notin \bar K$ and therefore $\chi(\alpha_0,p_0) \cap K= \emptyset$. Since $K$ is closed we can find small values $\tilde\alpha,\tilde p > 0$ such that $\chi(\alpha,p) \cap K = \emptyset$ for all $(\alpha,p)\in [\alpha_0, \alpha_0 + \tilde\alpha] \times [p_0, p_0 + \tilde p] =: \tilde R$. Hence we have $\tilde R\subset \Phi \setminus \chi^{-1}(\bar K)$, so $\Phi \setminus \chi^{-1}(\bar K)$ is open. Hence $\chi^{-1}(\bar K)$ is closed. \\
