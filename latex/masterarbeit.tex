...
where we can calculate $V_d(B_d) = \frac{\pi ^{\frac{d}{2}}}{\Gamma(\frac{d}{2} + 1)}$ with the $\mathit{Gamma\ function}$ $\Gamma$. 
...
\begin{definition}
	$V_0(K),\dots, V_d(K)$ are called $\mathit{intrinsic\ volumina}$ of $K$. 
\end{definition}
...
An interesting set in Integral Geometry is the set of $q$-dimensional affine spaces in the $d$-dimensional real space, short
\begin{flalign*}
A(d,q) := \{V \subset \R^d\ |\ V \text{ is an affine space with dimension }q\}
\end{flalign*}
the set of $q$-flats in $\R^d$. Later we will be interested in choosing random lines (i.e. $2$-flats). For that we need a suitable $\sigma$-algebra and measure on $A(d,q)$ which we'll construct in this section. 
...
In this paper we will discuss the case $d=2$ and $q=1$, hence the set of lines in $\R^2$. Note that every line $X\in A:=A(2,1)$ has the form $X=X_{a,b}:=\{a+tb\in \R^2\ |\ t\in \R^2\}$ for some vectors $a,b\in \R^2$ with $b\neq (0,0)$. For $r>0$ also define $B_r := B_r(0) := \{x\in \mathbb{R}^2\ |\ |x| \leq r\}$. 
