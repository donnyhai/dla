...
If for $A\in \mathcal{F}$ we have $\mathbb{P}(A)=1$ we will say that $\glqq A \text{ holds }\mathbb{P}\text{-a.s.}\grqq$, or short $\glqq A\text{ holds } \text{a.s.}\grqq$ ($A$ holds almost sure). Another short expression, if for a set of statements $(A_t)_{t\in I}$ with $I\in\{\N,\R\}$ we say $\glqq$$A_t$ holds for large $t$$\grqq$ it shall mean that there exists $T\in I$ such that $A_t$ holds for all $t>T$. 
...
in $\Z^d$ which seems to agree fairly well with simulations ($\cite{lawler}$ page 83). There are only few rigorously prooved results and we will see one of them in the following. First we will proof some lemmas. For that define two random variables
	...
	\end{flalign*}
Then it is easy to show that for all $n\in\N$ and $s\in [0,\infty)$ a.s.
	\begin{enumerate} \label{props}
		\item $r$ and $T$ grow monotonously 
		\item $T(r(n)) \leq n$
		\item $r(T(s)) \geq s$
	\end{enumerate}
hold. 
...
\begin{lemma} \label{randt}
	...
	\begin{enumerate}
		\item $\exists c>0: \{r(n) \leq ch(n) \text{ for large $n$}\}$ a.s.
		\item $\exists c>0: \{T(as)\geq ch^{-1}(s) \text{ for large $s$}\}$ a.s.
	\end{enumerate}
\end{lemma}

\begin{proof} 
	For the proof it is important to understand the facts that $T(s)\to \infty$ for $s\to \infty$ a.s. and also $r(n)\to \infty $ for $n\to \infty$ a.s.. \\
	$\Rightarrow$: 
	We will ommit writing $\glqq$a.s.$\grqq$. Take $c>0$ and choose $N\in\N$ such that $r(n)\leq ch(n)$ for all $n>N$. Hence there exists $\tilde c>0$ s.t. $\tilde ch^{-1}(r(n))\leq n$ for all $n>N$. We can choose $M\in\N$ s.t. $T(aM) > N$, hence $T(as)\geq T(aM) > N$ for all $s>M$ since $T$ grows monotonously. Hence we can write $\tilde c h^{-1}(r(T(as))) \leq T(as)$ for all $s>M$ and since $r(T(as))\leq as$ we finally get $\tilde c h^{-1}(a)h^{-1}(s)=\tilde ch^{-1}(as) \leq T(as)$ for all $s>M$ where we used the multiplicativity of $h$ and that $h^{-1}$ falls monotonously. \\
	$\Leftarrow$: 
	The proof for this direction works analogously.
\end{proof}

\begin{lemma} \label{geometric}
	Let $n\in\N$ and $T_1,\dots,T_n$ be independent geometric random variables with parameter $p<\frac{1}{2}$. Let $Y=T_1 + \dots  + T_n$, then for every $a\geq 2p$
	\begin{flalign*}
		\PP(Y\leq\frac{an}{p}) \leq (2e^2a)^n. 
	\end{flalign*}
\end{lemma}

\begin{proof}
	The proof of this lemma is presented in \cite{lawler} Lemma 2.6.2.
\end{proof}
...
\begin{theorem} \label{keytheorem}
	Let $0\in A\subset \Z^2$ be a connected set with $rad(A) = n$. Then there exists a $c>0$ such that
	\begin{flalign*}
		H_A(0) \leq cn^{-\frac{1}{2}}. 
	\end{flalign*}
\end{theorem}
\begin{proof}
...
\begin{proof}
	For $c>0$ define 
	\begin{flalign*}
		A_c := \{rad(\E_n) \leq cn^{\frac{2}{3}} \text{ for large }n\}
	\end{flalign*}
	and
	\begin{flalign*}
		D_c := \{T(2n) \geq c n^{\frac{3}{2}} \text{ for large }n \}.
	\end{flalign*}
	...
	\begin{flalign*}
		...
		&\leq \limsup_{n\to\infty} \frac{\ln(\int_{A_c} cn^{\frac{3}{2}} d\PP)}{\ln(n)} \\ 
		&= \limsup_{n\to\infty} \frac{\ln(cn^{\frac{3}{2}})}{\ln(n)} \\
		&= \frac{3}{2} \limsup_{n\to\infty} \frac{\ln(cn)}{\ln(n)} = \frac{3}{2}
	\end{flalign*}
	If we define
	\begin{flalign*}
	...
	\begin{flalign} \label{lambda}
		\PP(D_c) = 1. 
	\end{flalign}
	Note that $h$ is bijective, multiplicative and monotonously growing. For $n\in\N$ write $\E_n = \{y_1,\dots,y_n\}$ according to the definition in $\ref{incrementalaggregate}$, where $y_j$ is the $j$-th point added to the cluster. Let $\beta > 0$ which will be determined lateron. For $n\in\N$ let $\tilde m_n := \beta n^{\frac{3}{2}}$ and define 
	\begin{flalign*}
		V_n := \{T(2n) < \tilde m_n\}. 
	\end{flalign*}
	Further define the set of realised random walk paths of length $n$ with starting point in $\partial B_n$ 
	\begin{flalign*}
	Z_n := \{[z] := [z_1, \dots, z_n]\ |\ [z] \text{ is a realisation a of random walk with } z_1\in \partial B_n\}, 
	\end{flalign*}
	define events 
	\begin{flalign*}
		W_n([z]) := \{\exists j_1< \dots < j_n \leq \tilde m_n \ \text{ s.t. } z_i = y_{j_i} \text{ for all } i\in\{1,\dots,n\} \} \text{ for } [z]\in Z_n
	\end{flalign*}
	and the union of these events 
	\begin{flalign*}
		W_n := \bigcup_{[z] \in Z_n} W_n([z]). 
	\end{flalign*}
	Since the random walks are moving on the grid $\Z^2$, by $z_1\in\partial B_n$ we mean that $z_1$ is $\glqq$near$\grqq$ to $\partial B_n\subset\R^2$, precisely we could mean $z_1\in B_n\setminus B_{n-1}$. Anyway this detail will not create problems of relevant order. We will quickly proof that 
	\begin{flalign*}
	...
	\end{flalign*}
	we then can follow that $\PP(D_\beta) = 1$ and have finished the proof. So we want to show $\ref{borelcantelli}$. For that take $n\in \N$ and $[z]\in Z_n$ and define
	\begin{flalign*}
		\tau_i \text{ the index j s.t. } y_j = z_i
	\end{flalign*}
	and
	\begin{flalign*}
	...
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
	for some constant $c_1>0$. Define $a=\beta c_1$ and take $N\in\N$ s.t. $\beta n^{\frac{1}{2}} \geq 2$ and $p_n\leq 2$ for all $n>N$, then with $\tau_0 := 0$ and Lemma \ref{geometric} we have 
	\begin{flalign*}
		\PP(\tau_{n} \leq \beta n^{\frac{3}{2}}) = \PP(\sum_{i=0}^{n-1} \sigma_i \leq \beta c_1 n p_n^{-1}) \leq (2e^2\beta c_1)^{n} \quad \text{ for all } n>N
	\end{flalign*}
	and since $W_n([z]) \subset \{\tau_{n} \leq \beta n^{\frac{3}{2}}\}$ we get
	\begin{flalign*}
		\PP(W_n([z])) \leq (2e^2\beta c_1)^{n} \quad \text{ for all } n>N.
	\end{flalign*}
	...
	\begin{flalign*}
		\sum_{[z]\in Z_n} \PP(W_n([z])) \leq c_2n4^{n-1} (2e^2\beta c_1)^{n-1} = c_2 n(8e^2\beta c_1)^{n-1} \quad \text{ for all } n>N
	\end{flalign*} 
	and
	\begin{flalign*}
		\sum_{n>N} \PP(W_n) \leq \sum_{n>N} c_2 n(8e^2\beta c_1)^{n-1} =: r_\beta. 
	\end{flalign*}
	$r_\beta$ is finite if $8e^2\beta c_1<1$ and since $c_1$ was chosen independent of $\beta$ we can choose $\beta$ such that this holds. Therefore choosing $\beta$ accordingly in total we get
	\begin{flalign*}
		\sum_{n\in\N} \PP(W_n) < \infty, 
	\end{flalign*}
