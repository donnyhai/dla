...
Looking at computer simulations of External DLA in $\Z^2$ it seems that the clusters are relatively sparse and they appear to have a noninteger fractal dimension. In $\cite{magnetic}$ DLA is observed in a magnetic aggregation context, and empirically they find a fractal dimension of around $1.8$. Other simulations seem to suggest a value a little less than 1.7 for $d_f$ in two dimensions ($\cite{lawler}$ page 83). There is also a theory that predicts 
\begin{flalign*}
	d_f = \frac{d^2 + 1}{d+1}
\end{flalign*}
...
The following theorem gives a bound on the harmonic measure which will be necessary in the proof of the result for the growth rate of External DLA. The theorem is prooved by $\cite{lawler}$ in Theorem $2.5.2$ and we state it slightly different here to generalize it easier afterwards. 
...
\begin{theorem}
	For the growth rate $\alpha_f$ of External DLA in $\Z^2$ as defined in $\ref{notion}\ (\ref{growthrate})$ we have
	\begin{flalign}
...
\begin{proof}
	...
	\end{flalign*}
	and $a=2$, then by Lemma \ref{randt}, we can show (\ref{condi}) if there exists a constant $c>0$ such that 
	...
	\\ For a sequence of events $(A_n)_{n\in\N}$ recall this definition
	\begin{flalign*}
	...
	\end{flalign*}
	we can then conclude that $\PP(D_\beta) = 1$ and have finished the proof. So we want to show $(\ref{borelcantelli})$. For $n\in \N$, $[z]\in Z_n$ and $i\in \{1,\dots,n\}$ define random variables
	\begin{flalign*}
...
\begin{remark}
	With this theorem and (\ref{fractaldim}) we conclude that for the fractal dimension $d_f$ of External DLA we have $d_f \geq\frac{3}{2}$. In the next chapter we will look at another incremental aggregate which tries to approximate External DLA and compare simulations of both to get a hint on which growth rate of the two is bigger than or equal to the other one.
\end{remark}
