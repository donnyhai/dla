...
\begin{lemma}
	Define 
	\begin{flalign*}
		r: \N \to [0,\infty), n\mapsto rad(\E_n)
	\end{flalign*}
	and
	\begin{flalign*}
		T: [0,\infty) \to \N, r\mapsto \min\{j\in\N\ |\ r(j)\geq r\}.
	\end{flalign*}
	Then we have:
	\begin{itemize}
		\item $r$ and $T$ grow monotonously 
		\item $T(r(n)) \leq n$ for all $n\in\N$
		\item $r(T(r)) \geq r \text{ for all } r\in [0,\infty)$
	\end{itemize}
\end{lemma}
...
\begin{lemma}
	Let $a>0$ and $h:[0,\infty) \to [0,\infty)$ a bijective function. Then
	\begin{flalign*}
		\exists c>0: r(n) \leq ch(n) \text{ for large } n\quad  \Leftrightarrow \quad\exists c>0: T(ar)\geq ch^{-1}(r) \text{ for large } r
	\end{flalign*}
\end{lemma}
