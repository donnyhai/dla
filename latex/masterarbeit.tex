...
\begin{definition}
	Let $g=g_{\alpha,p}\in \G$ be a line. For a finite set $A\in \mathcal{P}_f$ we define 
	
	\begin{align*}
		L_{A} := \{ p\in A\ |\ L \text{ hits } p\}
	\end{align*}
	
	which is the subset of points in $A$ are hit by the line $L$ hits. For the following we suppose $g\cap A \neq \emptyset$. We want to define a total ordered relation $\triangleleft$ on $g\cap A$. We choose two points $(x_1,x_2),(y_1,y_2)\in g\cap A$ and devide the definition of a relation into four cases, which are the line going from left-bottom to right-top, left-top to right-bottom, parallel to the $x$-axis and parallel to the $y$-axis. Write $b=(b_1,b_2)$.  \\
	\\
...
\begin{lemma}
For a line $L=L_{a,b}\in \mathcal{L}$ and $A\in \mathcal{P}_f$ with $g\cap A\neq \emptyset$ the relation $\triangleleft$ on $g\cap A$ is totally ordered. 
	\begin{proof}
...
\begin{definition} $\mathit{Random\ Line\ Hitting\ Distribution}$\\
	...
	Let $L=L_{a,b}$ be a random line according to the line measure $\mu_0$ with the condition that $L$ hits $A$. Then define $\mu_A$ with distribution $\mu_A \sim U(\{min_{\triangleleft}g\cap A, max_{\triangleleft}g\cap A\})$, which chooses uniformly an element out of $\{min_{\triangleleft}g\cap A, max_{\triangleleft}g\cap A\}$. We call this distribution the $\mathit{Random\ Line\ Hitting\ Distribution\ (of\ A)}$.
\end{definition}
