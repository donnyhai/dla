...
\begin{flalign*}
	d_f = \frac{d^2 + 1}{d+1}
\end{flalign*}
in $\Z^d$ which seems to agree fairly well with simulations ($\cite{lawler}$ page 83). There are only few rigorously prooved results and one of them we will see in the following. First we will proof some lemmas. For that define two random variables
	\begin{flalign*}
...
\begin{proof}
	...
	\end{flalign*}
	we can follow that $\PP(B_\beta) = 1$ and have finished the proof. So we want to show $\ref{borelcantelli}$. For that take $n\in \N$ and $[z]\in Z_n$ and define
	\begin{flalign*}
	...
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
	for some constant $c_1>0$. With Lemma \ref{geometric} ...
