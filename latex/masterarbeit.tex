...
\begin{proof}
	...
	\begin{flalign*}
		...
		\end{cases}
	\end{flalign*}
	so $\sigma_i$ is the waiting time between adding $z_i$ and $z_{i+1}$ to the cluster. Consider the event $W_n([z])$ and take $i\in\{1,\dots,n-1\}$. Note that in this case $\tau_j$ is finite for all $j\in\{1,\dots,n\}$. Since $\E_{j_i}$ is connected and contains $0$, by Theorem $\ref{keytheorem}$ we get that the distribution of $\sigma_i$ is bounded by the one of a geometric random variable with parameter
	\begin{flalign*}
		p_n := c_1 n^{-\frac{1}{2}}
	\end{flalign*}
