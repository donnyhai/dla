...
For all incremental aggregates in this paper we will have $d=2$. From now on we will identify $\R^2$ with $\C$. 
...
\subsection{Construction in the plane}

For our special case we choose $d=2$ and $q=1$, thus lines in the plane. We denote this set of lines by $\G$. The following construction in this chapter is completely motivated by \cite{sackmann} 2.1.1. Firstly we propose a parametrisation of lines which works as follows. Every line can be uniquely determined by an angle $\alpha\in [0,\pi)$ and a real number $p\in \R$. Let $e_\alpha := e^{\alpha i} = \cos(\alpha) + \sin(\alpha)i$ be the unit vector $1$ rotated by $\alpha$ counterclockwise. It is easy to realize that $g_{\alpha,p} := \{x\in \C\ |\ \langle x,e_\alpha\rangle  = p\}$ defines a line and that every line has a unique pair of $\alpha$ and $p$ for such a representation (see in $\autoref{lineparam}$). With $\Phi := [0,\pi) \times \R$ this naturally defines a bijection
\begin{flalign*}
...
\end{lemma}
\begin{proof}
	To proof this it is helpful to consider generators of these $\sigma$-algebras. Define the set of closed rectangles in $\Phi$ as $R := \{[\alpha,\beta] \times [c,d]\subset \C\ |\ 0\leq \alpha<\beta<\pi,c<d\}$. Then by measure theory we know that $\mathcal{B}_\Phi = \sigma (R)$ and since $\chi$ is a bijection, we have $\chi(\sigma (R)) = \sigma (\chi(R))$ and finally $\GG = \sigma(\chi(R))$. For $\tilde A := \{A_K\ |\ K\in \K^2\}$ we have by definition $\mathcal{A}(2,1) = \sigma(\tilde A)$. \\
	\\ \indent $\subset$: Let $K\in\K^2$. We will show that $\chi^{-1}(A_K)$ is a closed set in $\Phi$. If that is the case we can write $\chi^{-1}(A_K) = \cup_{i=1}^\infty R_i$ for some rectangles $R_i\in R$, hence $A_K =  \cup_{i=1}^\infty \chi(R_i)$ and therefore $A_K\in\sigma(\chi(R)) = \GG$. Therefore we have $\tilde A\subset \GG$ and finally $\sigma(\tilde A) = \mathcal{A}(2,1) \subset \GG$. To show that $\chi^{-1}(A_K)$ is a closed let $(\alpha_0,p_0)\in \Phi\setminus \chi^{-1}(A_K)$. Then $\chi(\alpha_0,p_0) \notin A_K$ and therefore $\chi(\alpha_0,p_0) \cap K= \emptyset$. Since $K$ is closed we can find small values $\tilde\alpha,\tilde p > 0$ such that $\chi(\alpha,p) \cap K = \emptyset$ for all $(\alpha,p)\in [\alpha_0, \alpha_0 + \tilde\alpha] \times [p_0, p_0 + \tilde p] =: \tilde R$. Hence we have $\tilde R\subset \Phi \setminus \chi^{-1}(A_K)$, so $\Phi \setminus \chi^{-1}(A_K)$ is open. Hence $\chi^{-1}(A_K)$ is closed. \\
	\\ \indent $\supset$: CONTINUE
\end{proof}
...
\begin{lemma} \label{chi}
	Let $K=B_r\in\K^2$ and let $(\alpha,p)$ be uniformly distributed in $\tilde \Phi:=[0,\pi)\times [-r,r]=\chi^{-1}(A_K)\subset \Phi$. Then $\chi(\alpha,p)$ is a random $K$-isotropic line. 
\end{lemma}
\begin{proof}
	For $A\in\GG$ we get 
	\begin{flalign*}
...
For a line $g=g_{\alpha,p}\in\G$ and $A\in \mathcal{P}_f$ with $g\cap A\neq \emptyset$ the relation $\triangleleft$ on $g\cap A$ is totally ordered. 
	\begin{proof}
		...
		\\
		$\mathit{Transitivity:}$ For transitivity let $x \triangleleft y$ and $y \triangleleft z$. We find four cases. In case $Re(x) \neq Re(y)$ and $Re(y) \neq z.\text{real}$ we get $Re(x) < z.\text{real}$ by transitivity of $<$, hence $x \triangleleft z$. In case $Re(x)\neq Re(y)$ and $Re(y) = z.\text{real}$ we get $Re(x) < Re(y) = z.\text{real}$, therefore $x \triangleleft z$. In case $Re(x) = Re(y)$ and $Re(y) \neq z.\text{real}$ we get $Re(x) = Re(y) < z.\text{real}$, similair as the last case. In the last case $Re(x) = Re(y) = z.\text{real}$ we get $Im(x) < Im(y)$ and $Im(y) < z.\text{imag}$ and again by transitivity of $<$ we get $Im(x) < z.\text{imag}$, hence $x \triangleleft z$ again. \\
		\\
