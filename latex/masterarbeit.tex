...
In the next section we want to define an approximation for DLA. To do this we will need some concepts and results from Integral Geometry which we will discuss and develop in this section. An interesting set in Integral Geometry is the set of $q$-dimensional affine spaces in the $d$-dimensional real space, short
\begin{flalign*}
	A(d,q) := \{V \subset \R^d\ |\ V \text{ is an affine space with dimension }q\}
\end{flalign*}
In this paper we will discuss the case $d=2$ and $q=1$, hence the set of lines in $\R^2$. Note that every line $X\in A:=A(2,1)$ has the form $X=X_{a,b}:=\{a+tb\in \R^2\ |\ t\in \R^2\}$ for some vectors $a,b\in \R^2$ with $b\neq (0,0)$. Little Reminder: A set $A\subset \mathbb{R}^2$ is called $\mathit{convex}$ if and only if for any two points $a,b\in A$ we have that $\{ta + (1-t)b \in \mathbb{R}^2\ |\ t\in [0,1]\}\subset A$. For $r>0$ also define $B_r := B_r(0) := \{x\in \mathbb{R}^2\ |\ |x| \leq r\}$. 

\begin{definition}
	$\K^2 := \{K\subset \mathbb{R}^2\ |\ K\text{ is convex and compact }\}$\\
\end{definition}
